"""
PlsV2_with_visual_features.py
═══════════════════════════════════════════════════════════════════════════════
Fusion de PlsV2.py et score_assignment.ipynb

PRINCIPE:
  • Les features visuelles calculées (Variance, Brightness, etc.) remplacent
    les attributs binaires CelebA (-1/+1) comme "biomarqueurs".
  • La PLS reste supervisée par TARGET_ATTRIBUTE (Male/Female).
  • Les vecteurs de Pearson et le ranking d'importance opèrent sur les
    features continues au lieu des attributs catégoriels.
  • Le concept de Δr² du notebook biomarkers est transposé ici :
      Δr² = |alignement groupe Male| - |alignement groupe Female|
      → positif  : la feature discrimine davantage les hommes
      → négatif  : la feature discrimine davantage les femmes
═══════════════════════════════════════════════════════════════════════════════
"""
...
# Masques Male / Female pour l'analyse Δr² (analogue train/patient du biomarkers notebook)
mask_male   = np.array([lbl == 'Male'   for lbl in labels_text])
mask_female = np.array([lbl == 'Female' for lbl in labels_text])
...
# ==============================================================================
# 6. CALCUL DES VECTEURS DE PEARSON (features visuelles ← remplace df_attr)
# ──────────────────────────────────────────────────────────────────────────────
# Pour chaque feature, on calcule la corrélation de Pearson entre :
#   • la Composante PLS 1  (Z_supervised[:, 0])  → axe de discrimination Male/Female
#   • la Composante PLS 2  (Z_supervised[:, 1])  → variance orthogonale résiduelle
# et les valeurs de la feature pour toutes les images.
#
# La flèche (r_x, r_y) indique dans quelle direction de l'espace PLS
# la feature croit.  Sa longueur encode la force de l'association.
#
# On calcule aussi le Δr² (inspiré du notebook biomarkers) :
#   Δr² = |r²  sur les hommes| - |r² sur les femmes|
#   → mesure si la feature est plus/moins discriminante chez les hommes
# ==============================================================================
print("▶  Calcul des vecteurs de Pearson sur les features visuelles...")
...
pearson_vectors    = {}   # flèches pour le biplot (r_x, r_y)
alignment_all      = {}   # r² signé sur l'ensemble du test set
alignment_male     = {}   # r² signé sur les hommes seulement
alignment_female   = {}   # r² signé sur les femmes seulement
delta_alignment    = {}   # Δr² = |male| - |female|

for feat in FEATURE_NAMES:
    values = df_features_z[feat].values   # feature normalisée

    # Vecteur 2D pour le biplot
    r_x, _ = stats.pearsonr(pls_c1, values)
    r_y, _ = stats.pearsonr(pls_c2, values)
    pearson_vectors[feat] = (r_x, r_y)

    # Alignements par groupe (analogue train / patient du biomarkers notebook)
    r2_all  = pearson_r2_signed(pls_c1, values)
    r2_male = pearson_r2_signed(pls_c1[mask_male],   values[mask_male])
    r2_fem  = pearson_r2_signed(pls_c1[mask_female], values[mask_female])

    alignment_all[feat]    = r2_all
    alignment_male[feat]   = r2_male
    alignment_female[feat] = r2_fem
    delta_alignment[feat]  = abs(r2_male) - abs(r2_fem)

# Ajout du vecteur de référence pour TARGET_ATTRIBUTE (label binaire 0/1)
...
# Classement par |Δr²| décroissant
importance_rank = sorted(FEATURE_NAMES, key=lambda f: abs(delta_alignment[f]), reverse=True)
...
# ==============================================================================
# 8. FIGURE 2 — ALIGNEMENT PAR FEATURE (style biomarkers_identification)
# ──────────────────────────────────────────────────────────────────────────────
# Un panel par feature : scatter (PLS_comp1 × valeur feature) + droites de
# régression séparées pour les hommes et les femmes + Δr² affiché.
# Analogue exact de la Figure 2 du notebook biomarkers.
# ==============================================================================
print("▶  Génération des panels d'alignement par feature...")
...
for i, feat in enumerate(FEATURE_NAMES):
    ...
    feat_vals = df_features_z[feat].values
    ...
    ax.scatter(
        ...
        c='#e8a84a', s=8, alpha=0.5, linewidths=0, label='Female', zorder=1,
    )

    # Droites de régression
    xr_m, yr_m, r_m, p_m = fit_line(pls_c1[mask_male],   feat_vals[mask_male])
    xr_f, yr_f, r_f, p_f = fit_line(pls_c1[mask_female], feat_vals[mask_female])

    ax.plot(xr_m, yr_m, color='#63b3ed', lw=2.0, alpha=0.9,
            label=f'Male  r={r_m:.2f}')
    ax.plot(xr_f, yr_f, color='#e8a84a', lw=2.0, alpha=0.9, ls='--',
            label=f'Female r={r_f:.2f}')

    # Δr² — concept biomarkers transposé au contexte Male/Female
    delta  = delta_alignment[feat]
    d_col  = '#56de91' if delta > 0 else '#fc6e6e'
    d_sign = '▲' if delta > 0 else '▼'
    d_lbl  = 'Male > Female' if delta > 0 else 'Female > Male'

    ax.set_title(feat.replace('_Score', ''), fontsize=11, fontweight='bold', pad=6,
                 color='#c9d1d9')
    ax.set_xlabel('PLS Composante 1', fontsize=8, color='#8b949e')
    ax.set_ylabel(feat, fontsize=8, color='#8b949e')

    info = (f"Male:    r² = {abs(alignment_male[feat]):.3f}  |  {pval_str(p_m)}\n"
            f"Female:  r² = {abs(alignment_female[feat]):.3f}  |  {pval_str(p_f)}")
    ax.text(0.03, 0.97, info,
            transform=ax.transAxes, fontsize=7.5, va='top', ha='left',
            color='#c9d1d9',
            bbox=dict(boxstyle='round,pad=0.35', fc='#1c2128', ec='#30363d', alpha=0.85))

    ax.text(0.97, 0.05, f'{d_sign} Δr² = {abs(delta):.3f}  ({d_lbl})',
...
fig2.suptitle(
    ...
    "Δr² = |Male| - |Female|  (▲ = feature plus discriminante chez les hommes)",
...
# ==============================================================================
# 9. FIGURE 3 — RANKING D'IMPORTANCE DES FEATURES (|Δr²|)
# (analogue direct de la Figure 3 du notebook biomarkers)
# ==============================================================================
...
deltas = np.array([delta_alignment[f] for f in importance_rank])
colors = ['#56de91' if d > 0 else '#fc6e6e' for d in deltas]
labels_rank = [f.replace('_Score', '') for f in importance_rank]

bars = ax3.barh(labels_rank, deltas, color=colors, height=0.55,
                edgecolor='#21262d', linewidth=0.6)

for bar, val in zip(bars, deltas):
    sign = '▲ ' if val > 0 else '▼ '
    ax3.text(
        val + np.sign(val) * 0.003,
...
ax3.set_xlabel("Δr²  (|Male| − |Female|)  →  positif = feature plus alignée avec PLS chez les hommes",
               fontsize=10, color='#c9d1d9')
ax3.set_title(
    f"Ranking d'importance des features visuelles — Axe PLS '{TARGET_ATTRIBUTE}'",
...
p1 = mpatches.Patch(color='#56de91', label='Feature plus discriminante chez les hommes')
p2 = mpatches.Patch(color='#fc6e6e', label='Feature plus discriminante chez les femmes')
...
# ==============================================================================
# 10. FIGURE 4 — HEATMAP DE SYNTHÈSE (analogue Figure 4 du notebook biomarkers)
# Lignes : alignement All / Male / Female / Δr²
# Colonnes : chaque feature visuelle
# ==============================================================================
print("▶  Génération de la heatmap de synthèse...")

import matplotlib.ticker as ticker

fig4, ax4 = plt.subplots(figsize=(max(10, len(FEATURE_NAMES) * 1.1), 4.5))
fig4.patch.set_facecolor('#0e1117')
ax4.set_facecolor('#0d1117')

summary = np.array([
    [alignment_all[f]    for f in FEATURE_NAMES],
    [alignment_male[f]   for f in FEATURE_NAMES],
    [alignment_female[f] for f in FEATURE_NAMES],
    [delta_alignment[f]  for f in FEATURE_NAMES],
])
...
cbar4.set_label('r² signé  (− = anti-aligné,  + = aligné avec PLS 1)', fontsize=9,
                color='#c9d1d9')

ax4.set_title(
    f"Synthèse des alignements features / Axe PLS '{TARGET_ATTRIBUTE}'",
    fontweight='bold', pad=10, loc='left', color='white',
)
...
print('═' * 72)
print(f"  {'Feature':<26}  {'All r²':>8}  {'Male r²':>9}  {'Female r²':>11}  {'Δr²':>8}  {'Impact':>10}")
print('─' * 72)
for feat in importance_rank:
    r2_a  = alignment_all[feat]
    r2_m  = alignment_male[feat]
    r2_f  = alignment_female[feat]
    delta = delta_alignment[feat]
    impact = ('↑ MALE'   if delta >  0.05 else
              '↓ FEMALE' if delta < -0.05 else
              '≈ NEUTRE')
    print(f"  {feat.replace('_Score',''):<26}  {r2_a:>+8.3f}  {r2_m:>+9.3f}"
          f"  {r2_f:>+11.3f}  {delta:>+8.3f}  {impact:>10}")
print('═' * 72)
print()
top3 = importance_rank[:3]
print(f"  ⭑ TOP-3 features les plus discriminantes : {', '.join(t.replace('_Score','') for t in top3)}")
print()
