...
all_preds, all_targs = learn.get_preds(dl=test_dl, cbs=[GetLatentSpace()])
...
for i, feat in enumerate(FEATURE_NAMES):
    ...
    ax.scatter(
        classif_score[mask_female], feat_vals[mask_female],
        c='#444c56', s=6, alpha=0.4, linewidths=0, label='Female', zorder=1,
    )
    # Male en bleu mis en valeur
    ax.scatter(
        classif_score[mask_male], feat_vals[mask_male],
        c='#63b3ed', s=8, alpha=0.6, linewidths=0, label='Male', zorder=2,
    )

    # Droite de régression sur TOUTE la population
    xr, yr, r_full, p_full = fit_line(classif_score, feat_vals)
    ax.plot(xr, yr, color='#56de91', lw=2.0, alpha=0.9, zorder=3,
            label=f'r = {r_full:.2f} (tous)')

    # r² recalculé sur le score de classification (cohérent avec l'axe X)
    r_classif, p_classif = stats.pearsonr(classif_score, feat_vals)
    r2_classif = float(np.sign(r_classif) * r_classif**2)
    r2_col     = '#56de91' if r2_classif >= 0 else '#fc6e6e'
    r2_sign    = '▲' if r2_classif >= 0 else '▼'

    ax.set_title(feat.replace('_Score', ''), fontsize=11, fontweight='bold',
                 pad=6, color='#c9d1d9')
    ax.set_xlabel('Score de classification P(Male)', fontsize=8, color='#8b949e')
    ax.set_ylabel(feat, fontsize=8, color='#8b949e')

    # Boîte d'info : r² classif + p-value
    info = f"Tous  r² = {abs(r2_classif):.3f}  |  {pval_str(p_classif)}"
    ...
    ax.text(0.97, 0.05,
            f'{r2_sign} r²(classif) = {r2_classif:+.3f}',
...
fig2.suptitle(
    f"Alignement Features visuelles / Score de classification P('{TARGET_ATTRIBUTE}')\n"
...
# ==============================================================================
# 9. FIGURE 3 — RANKING D'IMPORTANCE DES FEATURES
# ──────────────────────────────────────────────────────────────────────────────
# Les r² sont recalculés ici sur le score de classification (classif_score)
# pour être cohérents avec l'axe X de la figure 2.
# alignment_classif remplace alignment_all pour le ranking et la heatmap.
# ==============================================================================
print("▶  Recalcul des alignements sur score de classification (fig3/fig4/rapport)...")

alignment_classif = {}
pval_classif      = {}
for feat in FEATURE_NAMES:
    values = df_features_z[feat].values
    r, p   = stats.pearsonr(classif_score, values)
    alignment_classif[feat] = float(np.sign(r) * r**2)
    pval_classif[feat]      = float(p)

# Nouveau ranking basé sur |r² classif|
importance_rank_classif = sorted(
    FEATURE_NAMES, key=lambda f: abs(alignment_classif[f]), reverse=True
)
...
r2_vals     = np.array([alignment_classif[f] for f in importance_rank_classif])
colors      = ['#56de91' if v >= 0 else '#fc6e6e' for v in r2_vals]
labels_rank = [f.replace('_Score', '') for f in importance_rank_classif]
...
ax3.set_xlabel(
    "r² signé  →  positif = feature augmente avec P(Male)  |  négatif = diminue",
    fontsize=10, color='#c9d1d9',
)
ax3.set_title(
    f"Ranking d'importance des features — Score de classification P('{TARGET_ATTRIBUTE}')",
...
p1 = mpatches.Patch(color='#56de91', label=f"Aligné positivement avec P('{TARGET_ATTRIBUTE}')")
p2 = mpatches.Patch(color='#fc6e6e', label=f"Anti-aligné avec P('{TARGET_ATTRIBUTE}')")
...
# ──────────────────────────────────────────────────────────────────────────────
# Utilise alignment_classif (r² vs score de classification) — même base
# que fig2 et fig3 pour la cohérence globale.
# ==============================================================================
...
# Une ligne : r² signé vs score de classification sur population complète
summary = np.array([[alignment_classif[f] for f in FEATURE_NAMES]])
...
cbar4.set_label(
    f"r² signé  (− = anti-aligné,  + = aligné avec P('{TARGET_ATTRIBUTE}'))",
    fontsize=9, color='#c9d1d9',
)

ax4.set_title(
    f"Synthèse alignements features / Score de classification P('{TARGET_ATTRIBUTE}')",
    fontweight='bold', pad=10, loc='left', color='white',
)
...
# 11. RAPPORT CONSOLE
# ──────────────────────────────────────────────────────────────────────────────
# Utilise alignment_classif et importance_rank_classif — cohérent avec fig2/3/4.
# ==============================================================================
print()
print('═' * 66)
print(f"  FEATURE IMPORTANCE REPORT — Score de classification P('{TARGET_ATTRIBUTE}')")
print('═' * 66)
print(f"  {'Feature':<26}  {'r²(classif)':>12}  {'Direction':>10}  {'p-value':>10}  {'Impact':>8}")
print('─' * 66)
for feat in importance_rank_classif:
    r2   = alignment_classif[feat]
    pval = pval_classif[feat]
    sign = '▲' if r2 >= 0 else '▼'
    ...
    print(f"  {feat.replace('_Score',''):<26}  "
          f"{sign}{abs(r2):>11.4f}  {direc:>10}  "
          f"{pval:>10.2e}{sig_marker}  {impact:>8}")
print('═' * 66)
print("  (* = significatif p < 0.05)")
print()
top3 = importance_rank_classif[:3]
